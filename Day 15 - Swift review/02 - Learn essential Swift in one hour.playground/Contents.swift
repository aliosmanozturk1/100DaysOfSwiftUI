...
/// Swift, nil birleştirme operatörü olarak adlandırılan üçüncü bir opsiyonel açma yöntemine sahiptir - bir opsiyonel açar ve opsiyonel boşsa varsayılan bir değer sağlar:
...
/// nil birleştirme operatörü, opsiyonellerin oluşturulduğu birçok yerde kullanışlıdır. Örneğin, bir string'ten bir integer oluşturmak, dönüştürme başarısız olabileceğinden bir optional Int? döndürür. Burada, varsayılan bir değer sağlamak için nil coalescing kullanabiliriz:
...
/// Opsiyonel zincirleme, opsiyonelleri opsiyonellerin içinde okur, bunun gibi:
...
/// Optional chaining 2. satırda var: bir soru işareti ve ardından daha fazla kod. "Eğer opsiyonun içinde bir değer varsa, onu aç ve sonra..." dememizi ve daha fazla kod eklememizi sağlar. Bizim durumumuzda "eğer diziden rastgele bir eleman aldıysak, onu büyük harfle yaz" diyoruz.
...
/// Hata verebilecek bir fonksiyonu çağırırken, try? seçeneğini kullanarak fonksiyonun sonucunu başarı durumunda bir değer içeren bir opsiyonel değere, aksi takdirde nil değerine dönüştürebiliriz.
...
/// getUser() fonksiyonu her zaman networkFailed atacaktır, ancak neyin atıldığı umurumuzda değildir - tek umursadığımız çağrının bir kullanıcıyı geri gönderip göndermediğidir.
